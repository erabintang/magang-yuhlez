...
<div class="min-h-screen flex items-center justify-center bg-zinc-50 py-12 px-4 sm:px-6 lg:px-8">
    <div class="max-w-md w-full space-y-8">
        <div>
            ...
            <h2 class="mt-6 text-center text-3xl font-bold text-zinc-900">Masuk ke YUHLEZ</h2>
            <p class="mt-2 text-center text-sm text-zinc-500">
                Gunakan akun Anda atau masuk dengan Google
            </p>
        </div>
        ...
        <form method="POST" action="{{ route('login.post') }}" class="space-y-4">
            ...
            <div class="flex items-center justify-end">
                <a href="{{ route('password.request') }}" class="text-sm text-yellow-600 hover:text-yellow-500">Lupa password?</a>
            </div>
        ...
        {{-- Google OAuth --}}
        <a href="{{ route('google.redirect') }}" class="w-full flex items-center justify-center px-4 py-3 border border-zinc-300 rounded-xl shadow-sm text-sm font-medium text-zinc-700 bg-white hover:bg-zinc-50 transition-colors">
        ...
        <div class="text-center space-y-3">
            <p class="text-sm text-zinc-500">Belum punya akun?</p>
            <div class="flex flex-col sm:flex-row gap-2 justify-center">
                <a href="{{ route('register.intern') }}" class="rounded-xl border border-zinc-300 px-4 py-2 text-sm font-medium text-zinc-700 bg-white hover:bg-zinc-50 transition-colors">
                    Daftar sebagai Intern
                </a>
                <a href="{{ route('register.company') }}" class="rounded-xl border border-yellow-400 px-4 py-2 text-sm font-medium text-yellow-700 bg-yellow-50 hover:bg-yellow-100 transition-colors">
