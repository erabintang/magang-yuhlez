...
@section('title', 'Daftar sebagai Perusahaan - YUHLEZ')
...
<div class="min-h-screen flex items-center justify-center bg-zinc-50 py-12 px-4 sm:px-6 lg:px-8">
    <div class="max-w-md w-full space-y-8">
        <div>
            ...
            <h2 class="mt-6 text-center text-3xl font-bold text-zinc-900">Daftar sebagai Perusahaan</h2>
            <p class="mt-2 text-center text-sm text-zinc-500">
                Ingin bermitra dengan YUHLEZ? Daftarkan perusahaan Anda
            </p>
        </div>
        ...
        <form method="POST" action="{{ route('register.company.post') }}" class="space-y-4">
            @csrf
            <div>
                <label for="name" class="block text-sm font-medium text-zinc-700 mb-1">Nama Perusahaan</label>
        ...
        {{-- Google OAuth for Company --}}
        <a href="{{ route('google.redirect', ['intent' => 'company']) }}" class="w-full flex items-center justify-center px-4 py-3 border border-zinc-300 rounded-xl shadow-sm text-sm font-medium text-zinc-700 bg-white hover:bg-zinc-50 transition-colors">
