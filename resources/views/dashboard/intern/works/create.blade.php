...
@section('page-title', 'Buat Karya Baru')
...
<div class="max-w-2xl">
    ...
    <div class="bg-white rounded-xl shadow-sm p-6">
        <h2 class="text-xl font-bold text-zinc-900 mb-1">Buat Karya Baru</h2>
        <p class="text-sm text-zinc-500 mb-6">Publikasikan karya terbaikmu ke halaman utama YUHLEZ.</p>
        ...
        <form action="{{ route('intern.works.store') }}" method="POST">
            @csrf
            <div class="space-y-4">
                <div>
                ...
                <div>
                    ...
                    <textarea name="description" rows="5"
                        class="w-full rounded-lg border border-zinc-300 px-3 py-2 text-sm focus:ring-2 focus:ring-yellow-400 focus:border-transparent outline-none">{{ old('description') }}</textarea>
                </div>
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        <label class="block text-sm font-medium text-zinc-700 mb-1">Kategori</label>
                        <input type="text" name="category" value="{{ old('category') }}" placeholder="Web, Mobile, AI..."
                            class="w-full rounded-lg border border-zinc-300 px-3 py-2 text-sm focus:ring-2 focus:ring-yellow-400 focus:border-transparent outline-none">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-zinc-700 mb-1">Tahun</label>
                        <input type="number" name="year" value="{{ old('year', date('Y')) }}" min="2020" max="{{ date('Y') + 1 }}"
                            class="w-full rounded-lg border border-zinc-300 px-3 py-2 text-sm focus:ring-2 focus:ring-yellow-400 focus:border-transparent outline-none">
                    </div>
                </div>
                <div>
                    <label class="block text-sm font-medium text-zinc-700 mb-1">URL Source Code</label>
                    <input type="url" name="source_code_url" value="{{ old('source_code_url') }}" placeholder="https://github.com/..."
                        class="w-full rounded-lg border border-zinc-300 px-3 py-2 text-sm focus:ring-2 focus:ring-yellow-400 focus:border-transparent outline-none">
                </div>
                <div>
                    <label class="block text-sm font-medium text-zinc-700 mb-1">URL Live Demo</label>
                    <input type="url" name="deploy_url" value="{{ old('deploy_url') }}" placeholder="https://..."
                        class="w-full rounded-lg border border-zinc-300 px-3 py-2 text-sm focus:ring-2 focus:ring-yellow-400 focus:border-transparent outline-none">
                </div>
            </div>
            <div class="mt-6 flex gap-3">
                <button type="submit" class="px-5 py-2 bg-yellow-400 text-zinc-900 text-sm font-semibold rounded-lg hover:bg-yellow-300 transition">Publikasikan</button>
