...
@section('page-title', 'Buat Karya - YUHLEZ')
...
<div class="mb-6">
    ...
    <h1 class="mt-2 text-2xl font-bold text-zinc-900">Buat Karya Baru</h1>
</div>

<form action="{{ route('company.works.store') }}" method="POST" class="max-w-2xl">
    @csrf
    ...
    <div class="space-y-5">
        ...
        <div>
            ...
            <input type="hidden" name="description" id="work-description-hidden" value="{{ old('description') }}">
            <div id="work-description-editor" data-wysiwyg="work-description-hidden" data-placeholder="Jelaskan detail karya ini..." class="wysiwyg-container"></div>
        </div>
        ...
        <div class="grid gap-4 sm:grid-cols-2">
            <div>
                ...
                <select name="category" id="category"
                    ...
                    <option value="Film & Video" {{ old('category') === 'Film & Video' ? 'selected' : '' }}>Film & Video</option>
            ...
            <div>
                ...
                <input type="number" name="year" id="year" value="{{ old('year', date('Y')) }}" min="2020" max="{{ date('Y') + 1 }}"
                    class="w-full rounded-xl border border-zinc-300 px-4 py-2.5 text-sm focus:border-yellow-400 focus:ring-2 focus:ring-yellow-400/20 outline-none">
            </div>
        </div>
        ...
        <div class="grid gap-4 sm:grid-cols-2">
            <div>
                ...
                <input type="url" name="source_code_url" id="source_code_url" value="{{ old('source_code_url') }}" placeholder="https://github.com/..."
                    class="w-full rounded-xl border border-zinc-300 px-4 py-2.5 text-sm focus:border-yellow-400 focus:ring-2 focus:ring-yellow-400/20 outline-none">
            </div>
            <div>
                <label for="deploy_url" class="block text-sm font-medium text-zinc-700 mb-1">Link Deploy / Live Demo</label>
                <input type="url" name="deploy_url" id="deploy_url" value="{{ old('deploy_url') }}" placeholder="https://..."
                    class="w-full rounded-xl border border-zinc-300 px-4 py-2.5 text-sm focus:border-yellow-400 focus:ring-2 focus:ring-yellow-400/20 outline-none">
            </div>
        </div>
        ...
        <div class="flex items-center gap-3">
            ...
            <input type="checkbox" name="is_published" id="is_published" value="1" {{ old('is_published', 0) ? 'checked' : '' }}
                class="h-4 w-4 rounded border-zinc-300 text-yellow-400 focus:ring-yellow-400">
            <label for="is_published" class="text-sm font-medium text-zinc-700">Publikasikan segera</label>
        </div>
    </div>

    <div class="mt-8 flex gap-3">
        <button type="submit" class="rounded-xl bg-yellow-400 px-6 py-2.5 text-sm font-semibold text-zinc-950 transition-colors hover:bg-yellow-300">Simpan Karya</button>
        <a href="{{ route('company.works.index') }}" class="rounded-xl border border-zinc-300 px-6 py-2.5 text-sm font-medium text-zinc-700 transition-colors hover:bg-zinc-50">Batal</a>
    </div>
</form>
@endsection
